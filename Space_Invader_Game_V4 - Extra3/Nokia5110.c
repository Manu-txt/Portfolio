...
#define DC                      (*((volatile uint32_t *)0x40005010))
#define DC_COMMAND              0
#define DC_DATA                 0x04
#define RESET                   (*((volatile unsigned long *)0x40005020))
#define RESET_LOW               0
...
void static lcdwrite(enum typeOfWrite type, char message){
	...
	// Clean up before enabling interrupts
	volatile int i;
	for (i = 0; i < 3; i++);
	DC = DC_DATA;
	EnableInterrupts();
...
void Nokia5110_Init(void){
  ...
  SYSCTL_RCGCSSI_R |= 0x00000004;  // activate SSI2
  SYSCTL_RCGCGPIO_R |= SYSCTL_RCGCGPIO_R1; // 1) activate clock for Port B
  delay = SYSCTL_RCGC2_R;               // allow time to finish activating
  GPIO_PORTB_DIR_R |= 0x0C;             // make PB2,3 out
  GPIO_PORTB_AFSEL_R |= 0xB0;           // enable alt funct on PB4,5,7
  GPIO_PORTB_AFSEL_R &= ~0x0C;          // disable alt funct on PA6,7
  GPIO_PORTB_DEN_R |= 0xBC;             // enable digital I/O on PA2,3,4,5,7
